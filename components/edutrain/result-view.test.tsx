...
describe("ResultView", () => {
  it("[S4-1] 세트 점수(정답 수/전체·백분율)가 표시된다", () => {
    ...
    render(
      <ResultView quizSet={mkQuizSet()} results={results} onRetryWeak={vi.fn()} onHome={vi.fn()} />,
    );
  ...
  it('[S4-4] "약점만 다시 풀기"를 누르면 onRetryWeak이 호출된다', async () => {
    ...
    render(
      <ResultView quizSet={mkQuizSet()} results={results} onRetryWeak={onRetryWeak} onHome={vi.fn()} />,
    );
